...
- **No text layer.** A scanned or image-exported PDF parses to nothing. Oracle's native path does not OCR.
- **Over the size limit.** Greenhouse stops parsing above 2.5MB. GulfTalent rejects uploads above 1MB.
...
- **Multi-column layouts.** Greenhouse names columned layouts as a cause of failed parsing. Extract the text and check whether your two columns interleave into nonsense.
...
- **Unusual letter spacing.** Greenhouse lists spacing between letters as a parse-failure cause. Tracking applied for design can split words character by character in the extraction.

## Will read badly to a human

The recruiter in Workday reads the parsed text, not your layout. So these matter even when parsing technically succeeds.
...
- **Inconsistent date formats.** Mixing `03/2021`, `Mar 2021` and `2021` across one document. Workday computes employment gaps and time in position from these automatically.
...
- **A filename like `CV FINAL v3 (updated) copy.pdf`.** SmartRecruiters advises short simple filenames with no spaces or special characters.
- **Tracked changes, comments or hidden revisions** left in a Word file.
- **Invisible page breaks** producing a blank page.

## Content and match

- **Does the CV use the words in the job advert?** Not stuffed, used. Where the advert says "stakeholder management" and the CV says "dealing with senior people", the words do not meet.
- **Are the employer's stated criteria addressed?** Years of experience, the specific qualification, the licence. This is what the Harvard research found people are actually filtered on.
